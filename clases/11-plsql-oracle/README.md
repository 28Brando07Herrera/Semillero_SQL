...
| Guion de clase (docente) | [guion.md](guion.md) |
| Instalación local de Oracle | [`recursos/entorno-local-oracle.md`](../../recursos/entorno-local-oracle.md) |

## Entorno

**[Oracle Live SQL](https://livesql.oracle.com)** — gratis, en el navegador, cuenta de Oracle gratuita. Es el `sqliteonline.com` de hoy.

El script base se pega y se corre con el botón **Run Script** (el de la hoja). El botón **Run** ejecuta una sola sentencia y va a dar error.
...
El script base y los ejemplos están escritos contra **Oracle Database 23ai** (Live SQL o Free). La sintaxis usada es conservadora a propósito —nada posterior a 12c salvo `FETCH FIRST` e `IDENTITY`— pero conviene **correr el script una vez antes de la clase** y comprobar los doce números de control.
